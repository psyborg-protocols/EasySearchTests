 ...
 * NEW: Draws the monthly sales chart inside the product modal.
  ...
  const ordersSamplesSearch = {
    init() {
      // Set default date range: last month to today
      const today = new Date();
      const oneMonthAgo = new Date();
      oneMonthAgo.setMonth(today.getMonth() - 1);
      document.getElementById('osStartDate').value = oneMonthAgo.toISOString().split('T')[0];
      document.getElementById('osEndDate').value = today.toISOString().split('T')[0];
      // Load dataset type select
      this.updateFieldVisibility();
      this.performSearch();
    },
    updateFieldVisibility() {
      const type = document.getElementById('osTypeSelect').value;
      const orderFields = ['osCustPO','osMinTotal','osMaxTotal'];
      orderFields.forEach(id => {
        const el = document.getElementById(id);
        if (type === 'Samples') {
          el.disabled = true;
          el.closest('.mb-3').style.opacity = '0.5';
        } else {
          el.disabled = false;
          el.closest('.mb-3').style.opacity = '1';
        }
      });
    },
    getDataSet() {
      const type = document.getElementById('osTypeSelect').value;
      return window.dataStore[type]?.dataframe || [];
    },
    performSearch() {
      const type = document.getElementById('osTypeSelect').value;
      const data = this.getDataSet();
      const start = new Date(document.getElementById('osStartDate').value);
      const end = new Date(document.getElementById('osEndDate').value);
      const orderNo = document.getElementById('osOrderNo').value.trim().toLowerCase();
      const cust = document.getElementById('osCustomer').value.trim().toLowerCase();
      const custPO = document.getElementById('osCustPO').value.trim().toLowerCase();
      const minTotal = parseFloat(document.getElementById('osMinTotal').value) || -Infinity;
      const maxTotal = parseFloat(document.getElementById('osMaxTotal').value) || Infinity;

      // Filter rows
      let results = data.filter(row => {
        // unify field names for samples vs orders
        const rowOrderNo = String(row['Order No.'] || row['Order'] || '').toLowerCase();
        const rowCust = String(row['Customer'] || '').toLowerCase();
        const rowDate = new Date(row['Order Date'] || row['Customer order(date)']);
        const rowPO = String(row['Cust PO'] || '').toLowerCase();
        const rowTotal = parseFloat(row['Invoice Total'] ?? row['Total'] ?? 0);

        // match user inputs
        const matchOrder = !orderNo || rowOrderNo.includes(orderNo);
        const matchCust = !cust   || rowCust.includes(cust);
        const matchDate = (!isNaN(start) && !isNaN(end)) ? (rowDate >= start && rowDate <= end) : true;
        const matchPO   = type === 'Orders' ? (!custPO || rowPO.includes(custPO)) : true;
        const matchTotal = type === 'Orders' ? (rowTotal >= minTotal && rowTotal <= maxTotal) : true;

        return matchOrder && matchCust && matchDate && matchPO && matchTotal;
      });

      // sort descending by date
      results.sort((a, b) => new Date(b['Order Date'] || b['Customer order(date)']) -
                            new Date(a['Order Date'] || a['Customer order(date)']));

      this.updateTable(results, type);
    },
    updateTable(results, type) {
      const tbody = document.getElementById('ordersSamplesTable');
      if (results.length === 0) {
        tbody.innerHTML = `<tr><td colspan="5" class="text-muted fst-italic">No records found</td></tr>`;
        return;
      }
      tbody.innerHTML = results.map(row => {
        const orderNo  = row['Order No.'] ?? row['Order'];
        const cust     = row['Customer'];
        const orderDt  = row['Order Date'] ?? row['Customer order(date)'];
        const po       = row['Cust PO'] ?? '';          // sample rows won’t have this
        const total    = row['Invoice Total'] ?? row['Total'] ?? '';
        return `
          <tr class="order-row" onclick="UIrenderer.ordersSamplesSearch.showDetail(${encodeURIComponent(JSON.stringify(row))}, '${type}')">
            <td>${orderNo}</td>
            <td>${cust}</td>
            <td>${new Date(orderDt).toLocaleDateString('en-US')}</td>
            <td>${po}</td>
            <td>${total}</td>
          </tr>`;
      }).join('');
    },
    showDetail(rowStr, type) {
      // rowStr arrives as encoded JSON; parse it
      const row = JSON.parse(decodeURIComponent(rowStr));
      const modalBody = document.getElementById('osDetailBody');
      // Build a definition list or table to show all columns
      const rows = Object.keys(row).map(key => {
        return `<div class="row mb-1">
          <dt class="col-sm-4">${key}</dt>
          <dd class="col-sm-8">${row[key] ?? ''}</dd>
        </div>`;
      }).join('');
      modalBody.innerHTML = rows;
      new bootstrap.Modal(document.getElementById('osDetailModal')).show();
    }
  };
  ...
  document.getElementById('osTypeSelect').addEventListener('change', () => {
    ordersSamplesSearch.updateFieldVisibility();
  });
  document.getElementById('osSearchBtn').addEventListener('click', () => {
    ordersSamplesSearch.performSearch();
  });
